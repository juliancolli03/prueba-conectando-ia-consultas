...
router.post('/', async (req, res, next) => {
  try {
    ...
    if (!category && data.message) {
      req.log.info('Classifying message with AI...');
      try {
    ...
    try {
      ...
      return res.status(503).json({
        ...
        message: 'No se pudo procesar el lead. Por favor intenta más tarde.'
      });
    }
    ...
    try {
      ...
      }, 'Sending lead to notifications service');
      ...
      // No fallamos la request si notifications falla
    }

    // Respuesta exitosa
    res.status(leadResponse.status === 'created' ? 201 : 200).json({
      ok: true,
      status: leadResponse.status,
      leadId: leadResponse.lead._id || leadResponse.lead.id,
      createdAt: leadResponse.lead.createdAt,
      updatedAt: leadResponse.lead.updatedAt,
      notified
    });
